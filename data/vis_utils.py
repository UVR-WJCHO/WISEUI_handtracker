    ...
    ### 35 >> 0.2
        ...
        for joint_num in range(gtIn.shape[0]):
            ...
            if joint_num in [0, 4, 8, 12, 16]:
                if PYTHON_VERSION == 3:
                    joint_color = list(map(lambda x: x + 0.2 * (joint_num % 4), joint_color_code[color_code_num]))
                else:
                    joint_color = map(lambda x: x + 0.2 * (joint_num % 4), joint_color_code[color_code_num])
        ...
        for limb_num in range(len(limbs)):
            ...
            if length > 5:
                ...
                if PYTHON_VERSION == 3:
                    limb_color = list(map(lambda x: x + 0.2 * (limb_num % 4), joint_color_code[color_code_num]))
                else:
                    limb_color = map(lambda x: x + 0.2 * (limb_num % 4), joint_color_code[color_code_num])
...
def showHandJoints_vis(imgInOrg, gtIn, vis=None, filename=None):
    ...
    # Set color for each finger
    # joint_color_code = [[0, 255, 255], [255, 0, 0],
    #                     [255, 0, 255]]

    joint_color_code = [[0, 56/255.0, 255/255.0],
    ...
    if gtIn.shape[0]==1:
        ...
        for limb_num in range(len(limbs)):
            ...
            if length > 5:
                ...
                color_code_num = limb_num // 4
                if PYTHON_VERSION == 3:
                    limb_color = list(map(lambda x: x + 35 * (limb_num % 4) / 255., joint_color_code[0]))
                else:
                    limb_color = map(lambda x: x + 35 * (limb_num % 4) / 255., joint_color_code[0])
        ...
        for joint_num in range(gtIn.shape[0]):
            if joint_num is 0:
                ...
                if PYTHON_VERSION == 3:
                    joint_color = list(map(lambda x: x + 35 * (joint_num % 4) / 255., joint_color_code[2]))
                else:
                    joint_color = map(lambda x: x + 35 * (joint_num % 4) / 255., joint_color_code[2])

                cv2.circle(imgIn, center=(gtIn[joint_num][0], gtIn[joint_num][1]), radius=3, color=joint_color, thickness=1)
            else:
                if PYTHON_VERSION == 3:
                    joint_color = list(map(lambda x: x + 35 * (joint_num % 4) / 255., joint_color_code[1]))
                else:
                    joint_color = map(lambda x: x + 35 * (joint_num % 4) / 255., joint_color_code[1])
